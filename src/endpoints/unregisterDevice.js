...
module.exports.run = async (request, database) => {
    ...
    try {
        registeredDevice = (await query(database, "SELECT * FROM Registered_Devices WHERE MAC_Address=?", [mac]))[0];
    } catch (error) {
    ...
    try {
        await query(database, "DELETE FROM Registered_Devices WHERE MAC_Address=?", [registeredDevice.MAC_Address]);
    } catch (error) {
        request.end(500, "Internal server error");
        console.log(`SQL Error - ${__filename} - ${error}`);
        return;
